 ...
 *            (C)  2018 Ryan Andri <https://github.com/ryan-andri>
 *
...
#define DEF_FREQ_STEP_DOWN			(7)
#define DEF_DOWN_DIFFERENTIAL			(4)
#define DEF_IO_IS_BUSY				(0)

/*
 * No need fixed frequency since __cpufreq_driver_target is smart away!
 */
#define DEF_SUSPEND_MAX_FREQ			(980000)
...
struct cpu_dbs_info_s {
	...
	struct workqueue_struct *clarity_gov_wq;
...
static DEFINE_PER_CPU(struct cpu_dbs_info_s, cg_cpu_dbs_info);
		     ...
		     void *data)
{
	...
	struct cpu_dbs_info_s *this_dbs_info = &per_cpu(cg_cpu_dbs_info,
							freq->cpu);
...
static void update_sampling_rate(unsigned int new_rate)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (!policy)
			continue;
		dbs_info = &per_cpu(cg_cpu_dbs_info, policy->cpu);
		...
		if (time_before(next_sampling, appointed_at)) {
			...
			queue_delayed_work(dbs_info->clarity_gov_wq,
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		struct cpu_dbs_info_s *dbs_info;
		dbs_info = &per_cpu(cg_cpu_dbs_info, j);
			       ...
			       const char *buf, size_t count)
{
	...
	for_each_online_cpu(cpu) {
		struct cpu_dbs_info_s *dbs_info;
		dbs_info = &per_cpu(cg_cpu_dbs_info, cpu);
		dbs_info->prev_cpu_idle = get_cpu_idle_time(cpu,
						&dbs_info->prev_cpu_wall,
						dbs_tuners_ins.io_is_busy);
	}
...
/************************** sysfs end ************************/
static unsigned int check_cpu_load(struct cpufreq_policy *policy)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		j_dbs_info = &per_cpu(cg_cpu_dbs_info, j);
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	/* Calculate CPU's Load per current policy */
	max_load = check_cpu_load(policy);
...
static inline int dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	int ret = 0, delay = 0;

	dbs_info->clarity_gov_wq = alloc_workqueue("clarity_gov_wq", WQ_HIGHPRI, 0);
	if (!dbs_info->clarity_gov_wq) {
		pr_err("%s: Failed to create clarity_gov workqueue\n", __func__);
		ret = -ENOMEM;
		goto out;
	}
...
static inline int dbs_timer_exit(struct cpu_dbs_info_s *dbs_info)
{
	dbs_info->enable = 0;
	cancel_delayed_work_sync(&dbs_info->work);
	destroy_workqueue(dbs_info->clarity_gov_wq);
				   ...
				   unsigned int event)
{
	...
	this_dbs_info = &per_cpu(cg_cpu_dbs_info, cpu);
	...
	case CPUFREQ_GOV_START:
		...
		if (dbs_enable == 1) {
			...
			/*
			 * clarity does not implement micro like ondemand
...
MODULE_DESCRIPTION("'cpufreq_clarity' - A dynamic cpufreq governor for "
